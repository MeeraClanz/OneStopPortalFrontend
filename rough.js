...
const Login = () => {
  ...
  const handleLogin = (payload) => {
    setIsLoading(true);
    axios
      .post(constants.URL.SIGNIN, payload)
      .then((resp) => {
        setUser(resp.data.results);
        // Store email in localStorage for future logins
        if (resp.data.results?.email) {
          localStorage.setItem("userEmail", resp.data.results.email);
        }
  ...
  const onSubmit = (data) => {
    ...
    localStorage.setItem("userEmail", data.userEmail); // Use data.userEmail here
    navigate("/TopBar");
  };
